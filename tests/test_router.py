...
class TestModelRouter:
    ...
    def test_requires_cloud_flag_alone_does_not_route_to_claude(self):
        # requires_cloud is an IntentClassifier guess, not a complexity signal --
        # it must not override the local-first default outside COMPLEX_CATEGORIES.
        assert self.router.choose("INFORMATION", {"requires_cloud": True}, "what's the meaning of life") == Backend.LOCAL
    ...
    def test_reservation_with_requires_cloud_still_routes_to_local(self):
        assert self.router.choose("RESERVATION", {"requires_cloud": True}, "book a table") == Backend.LOCAL
    ...
    def test_code_category_routes_to_claude_regardless_of_requires_cloud(self):
        # COMPLEX_CATEGORIES (explicit complex coding/deep reasoning) is the
        # only thing that escalates to Claude now -- confirm requires_cloud=False
        # on an explicit CODE category still correctly goes to Claude.
        assert self.router.choose("CODE", {"requires_cloud": False}, "build me a website") == Backend.CLAUDE
